# Spec — Run History Domain
**Change**: run-history-persistence (SDD#3)
**Domain**: run-history (new capability)
**Phase**: spec (promoted from change → capability on 2026-06-11)
**Date**: 2026-06-11

> **Promotion note**: This spec was developed under SDD#3 (`run-history-persistence`)
> and promoted to a standing capability spec after all four PRs (#66/#67/#68/#69) merged
> to `main`. The implementation note for RH-008 (completed_at-based sweep cutoff) reflects
> the PR-1 design decision recorded in the change artifacts.

---

## Purpose

This spec covers the `run-history` capability: durable per-run metadata persistence,
...
The following are absolute prohibitions for all implementation work in this capability:
...
- `application/pipeline.py` MUST import only Protocols (ports) and config/run_context — zero
  concrete adapter imports. Manifest write boundary is at the `_run_pipeline_background`
  wrapper in `routes.py` (not the pipeline).

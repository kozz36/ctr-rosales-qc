...
Contract:
  ...
  - ``read_material_table`` always returns ``[]`` -- REV-R10 (T3) Null Object stub.
...
guia, ``_stage_normalize_dates`` falls into the ``day is None or month is None``
branch (R9b Rule-2) and applies ``apply_delivery_floor(None, fecha_entrega)``,
resolving each guia's fecha to its SUNAT ``fecha_entrega`` (``delivery_floor_applied=True``).
...
class NullVisionAdapter:
    ...
    def read_material_table(
        ...
        """Return an empty list -- no LLM call, no IO (REV-R10, Null Object pattern).

        Vision-off graceful stub: when vision.enabled=False, apply_reprocess still
        returns vision_empty (recovered=False) rather than crashing.
        """
        return []

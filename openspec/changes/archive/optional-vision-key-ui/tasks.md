...
- [x] **7.3** — Judgment-day adversarial review before push. Focus: key-never-logged invariant, port boundary (no concrete adapter import in application/), lazy openai import, domain purity, fail-fast not triggerable via invalid key + SUNAT-off. **COMPLETE**: dual-blind JD phase identified CRITICAL-1 openai.Timeout non-exception (JD fix applied; main commit 5dab423 includes fix key_probe.py:106); HIGH-2 atomic-write dir-perms (fixed vision_key_file_store.py:97-99 O_EXCL+0600+dir 0o700); HIGH-3 PROVIDER default missing in lifespan (fixed main.py:68-72 force ENABLED+API_KEY, setdefault PROVIDER/BASE_URL/MODEL); HIGH-4 DELETE route (implemented routes.py:2253). All real-SDK tests present (429 RateLimitError, 404 NotFoundError); HIGH-2 proves CRITICAL-1 dead. Evidence in engram `ctr-rosales-qc/reviews/sdd4-pr1-judgment`.
...
- [x] **12.1** — Start app locally (backend + frontend). Navigate to review UI with `vision.enabled=false` (default keyless). Assert: all three reprocess surfaces visible and disabled; hover shows tooltip; clicking disabled button triggers zero network requests (REV-R34-S01/S03/S04). **COMPLETE**: Surface 1 (GuiaDrillDown Reprocesar) runtime PASS. Evidence: `docs/playwright/sa5-vision-key-pr2.json` (flow: vision-off → surface disabled+tooltip, modal empty-key error, invalid-key error, confirm disabled click triggers zero calls).
- [x] **12.2** — Open hamburger menu → click \"Ajustes\" → assert modal opens (VKS-004-S03). Submit empty key → assert error. **COMPLETE**: Modal-flow and error-path runtime PASS (sa5-vision-key-pr2.json).
- [x] **12.3** — Submit an invalid key via modal → assert error state with backend message; `GET /capabilities` still returns `vision_enabled: false` (VKS-004-S02, VKS-001-S02). **COMPLETE**: Invalid-key error path runtime PASS (sa5-vision-key-pr2.json).
- [x] **12.4 (real-data gate — real Ollama Cloud key required)** — Submit a valid Ollama Cloud key → assert success state + restart notice + key field cleared (VKS-004-S01). Restart backend → `GET /capabilities` returns `vision_enabled: true`. Navigate to reprocess → all three surfaces interactive. Trigger a real reprocess to confirm end-to-end. **NOTE**: Surfaces 2 (ErroredGuiasPanel) and 3 (PendientesPorProcesarTab) validated by unit tests only — SA-5 OCR+SUNAT run produced zero errored guías, so panels v-if-hidden. Gating source verified by code inspection (:disabled+tooltip present). Residual risk noted in verify-report W-RUNTIME.
...
- [x] **13.3** — Judgment-day adversarial review before push. Focus: `v-if` vs `disabled` (no DOM removal), store reactivity (REV-R35-S02), key input never pre-populated, modal error path, SA-5 evidence completeness. **COMPLETE**: ctr-reviewer APPROVE-WITH-FINDINGS on PR-2 frontend (engram `ctr-rosales-qc/reviews/sdd4-pr2-frontend`). Finding: W-RUNTIME surfaces 2/3 (noted in verify); all gating logic + modal flows verified. Ready to merge.
